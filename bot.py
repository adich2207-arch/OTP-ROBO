...
# Global PTB application (built once at startup)
...
# Admin approves deposit via /approve_<id>
async def admin_approve_deposit(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Deposit #{dep_id} approved. ${dep['amount']:.2f} credited.")
    await ctx.bot.send_message(
        dep["user_id"],
...
async def buy_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = []
    for l in listings:
        label = f"@{l['tg_username']} — ${l['price']:.2f}"
        buttons.append([InlineKeyboardButton(label, callback_data=f"buy_{l['id']}")])
...
async def buy_item(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                "JOIN users u ON l.seller_id=u.user_id WHERE l.id=%s AND l.status='active'",
                (listing_id,)
            )
    ...
    await query.edit_message_text(
        ...
        f"{'✅ You have enough funds.' if balance >= listing['price'] else '❌ Insufficient balance. Please deposit first.'}",
        parse_mode="Markdown",
        reply_markup=keyboard
    )
...
async def buy_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("UPDATE listings SET status='sold', buyer_id=%s WHERE id=%s",
                        (user.id, listing_id))
            cur.execute(
...
async def my_listings(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        lines = ["📋 *Your Listings*\n"]
        for r in rows:
            status_icon = {"active": "🟢", "sold": "✅", "cancelled": "🔴"}.get(r["status"], "⚪")
            lines.append(f"{status_icon} @{r['tg_username']} — ${r['price']:.2f} ({r['status']})")
        text = "\n".join(lines)
